...
func (aq *AccountQuery) QueryOauth() *OAuthQuery {
	query := &OAuthQuery{config: aq.config}
...
func (aq *AccountQuery) WithOauth(opts ...func(*OAuthQuery)) *AccountQuery {
	query := &OAuthQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withOauth; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Oauth = []*OAuth{}
		}
		query.withFKs = true
		query.Where(predicate.OAuth(func(s *sql.Selector) {
			s.Where(sql.InValues(account.OauthColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_oauth
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_oauth" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_oauth" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Oauth = append(node.Edges.Oauth, n)
		}
	}
