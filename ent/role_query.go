...
func (rq *RoleQuery) QueryApplication() *ApplicationQuery {
	query := &ApplicationQuery{config: rq.config}
...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withFuncs; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(role.FuncsPrimaryKey[0], fks...))
			},
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "funcs" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Funcs = append(nodes[i].Edges.Funcs, n)
			}
		}
	}
