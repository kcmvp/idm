...
func (oq *OAuthQuery) QueryAccount() *AccountQuery {
...
func (oq *OAuthQuery) Clone() *OAuthQuery {
	...
	return &OAuthQuery{
		...
		withAccount: oq.withAccount.Clone(),
...
func (oq *OAuthQuery) WithAccount(opts ...func(*AccountQuery)) *OAuthQuery {
	query := &AccountQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withAccount = query
	return oq
}
...
func (oq *OAuthQuery) sqlAll(ctx context.Context) ([]*OAuth, error) {
	...
	if query := oq.withAccount; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*OAuth)
		for i := range nodes {
			if nodes[i].account_oauth == nil {
				continue
			}
			fk := *nodes[i].account_oauth
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_oauth" returned %v`, n.ID)
			}
			for i := range nodes {
