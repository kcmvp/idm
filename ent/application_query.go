...
func (aq *ApplicationQuery) sqlAll(ctx context.Context) ([]*Application, error) {
	...
	if query := aq.withRoles; query != nil {
		...
		query.Where(predicate.Role(func(s *sql.Selector) {
			s.Where(sql.InValues(application.RolesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.application_roles
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "application_roles" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "application_roles" returned %v for node %v`, *fk, n.ID)
			}
